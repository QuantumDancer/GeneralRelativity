...
For a fourdimensional space\footnote{this does certainly not hold true for
$d>4$} the most general divergence free tensor $\tensor{A}{_\mu_\nu}$ is given by
\begin{equation}
\tensor{A}{_\mu_\nu}= c_1\tensor{G}{_\mu_\nu}+c_2\tensor{g}{_\mu_\nu}\, .
\end{equation}
...
line element $\dif s^2$ has dimension, ${[\dif s^2]=\textrm{M}^{-2}}$\footnote{Where M referes to the dimenson of mass.}.
Since further ${\left[\tensor{x}{^\mu}\right]=\textrm{M}^{-1}}$ ,
the Lagrange density must have Dimension ${\left[\mathcal{L}\right]=\textrm{M}^{4}}$
\end{remark}
...
\end{equation}
%TODO consider possible wrong sign
\begin{remark}
Attention $\delta_g
...
\end{enumerate}
%TODO part about Intrinsic vs extrinsic curvature 
...
\end{figure}
%TODO explanation what happens!
Assume we know eighter
\begin{itemize}
  \item $\tensor{g}{_\mu_\nu}$ on $\Sigma_{t_0}$
  \item $\tensor{g}{_\mu_\nu_{;j}}$, $\tensor{g}{_\mu_\nu_{;0}}$ on
  $\Sigma_{t_0}$
