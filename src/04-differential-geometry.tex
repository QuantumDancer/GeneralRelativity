...
\end{remark}
(Evtl. Konstruktion, wird im skript angedeutet, auch geodesic coordinates\ldots)
We notice that if 
\begin{equation}
...
beeing threedimensional gravity.
\end{sidenote}
Inner product 
\begin{equation}
...
In curved coordinates we have (warum?)
\begin{equation}
...
\end{remark}
(Nochmal variation der Bogenlänge\ldots)
...
A general metric compatible connection ($\tensor{\nabla}{_k}\tensor{g}{_i_j}=0$)
satisfies
\begin{equation}
...
\end{remark}
\begin{equation}
\tensor{R}{ijkl}=\frac{1}{2}\left(\dmd{{\tensor{g}{_i_l}}}{2}{x}{}{x}{}\right)
\end{equation}